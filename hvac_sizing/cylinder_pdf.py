...
def date_time(value: object) -> str:
    try:
...
def cylinder_story(cylinder: dict, target_url: str, style: dict) -> list:
    capacity = "—" if cylinder.get("capacity_kg") is None else kg(cylinder["capacity_kg"])
    notes = safe(cylinder.get("notes")) or "—"
    header_text = [
        Paragraph("SCHEDA BOMBOLA REFRIGERANTE", style["subtitle"]),
        Paragraph(safe(cylinder["name"]), style["title"]),
        Paragraph(f"Codice: <b>{safe(cylinder['code'])}</b> &nbsp;&nbsp; Refrigerante: <b>{safe(cylinder['refrigerant'])}</b>", style["body"]),
        Spacer(1, 2 * mm),
        Paragraph("Scansiona il QR per aprire la scheda e registrare pesature, aggiunte o prelievi.", style["small"]),
    ]
    ...
    balance = Table([
        [Paragraph("GAS EFFETTIVO RESIDUO", style["balance_label"]), Paragraph("PESO TOTALE", style["balance_label"])],
    ...
    details = Table([
        ["Tara bombola", kg(cylinder["tare_kg"]), "Capacità gas", capacity],
        ["Creata", date_time(cylinder["created_at"]), "Ultimo aggiornamento", date_time(cylinder["updated_at"])],
        ["Note", Paragraph(notes, style["body"]), "", ""],
    ...
    operation_names = {"initial": "Registrazione iniziale", "weighing": "Pesatura", "add": "Aggiunta", "remove": "Prelievo"}
    history_rows = [["Data", "Operazione", "Quantità / peso", "Residuo", "Note"]]
    for item in cylinder.get("history", []):
        if item["operation"] == "weighing":
            measure = f"Totale {kg(item.get('total_weight_kg'))}"
        else:
    ...
    if len(history_rows) == 1:
        history_rows.append(["—", "Nessun movimento", "—", kg(cylinder["current_gas_kg"]), "—"])
    ...
    return [
        ...
        Spacer(1, 4 * mm),
        details,
        Paragraph("STORICO MOVIMENTI", style["heading"]),
        history,
    ]
...
def _footer(canvas, document) -> None:
    ...
    canvas.drawString(17.5 * mm, 10 * mm, "Dimensionamento Climatizzazione Pro · Magazzino bombole")
    canvas.drawRightString(192.5 * mm, 10 * mm, f"Pagina {document.page}")
    canvas.restoreState()


def generate_cylinder_pdf(cylinders: list[dict], base_url: str) -> bytes:
    output = io.BytesIO()
    document = SimpleDocTemplate(
        ...
        topMargin=15 * mm,
        bottomMargin=16 * mm,
        title="Schede bombole refrigerante",
    ...
    for index, cylinder in enumerate(cylinders):
        ...
        story.extend(cylinder_story(cylinder, target, style))
    if not story:
        story.append(KeepTogether([Paragraph("Nessuna bombola registrata", style["title"])]))
